...
fn publish(networks: Vec<ChainInfo>) -> anyhow::Result<()> {
    ...
    for network in networks {
        ...
        let app_namespace = Namespace::from_id(FUSOGEN_ID)?;
        ...
        let publisher_acc = abstract_client.fetch_or_build_account(app_namespace, |builder| builder.namespace(Namespace::from_id(FUSOGEN_ID).unwrap()))?;
        ...
        publisher.publish_app::<FusogenInterface<Daemon>>()?;
    }
    Ok(())
}
